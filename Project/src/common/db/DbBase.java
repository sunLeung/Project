...
public abstract class DbBase {
	 ...
	 * @throws SQLException
	 */
	public int stat(String sql,
			Object... params) throws SQLException {
		Connection conn=getConnection();
		try {
			this.startTransaction(conn);
			int i=((Number) runner.query(conn, sql, scaleHandler,
					params)).intValue();
	...
	public int insertOracle(Pojo obj,String indexSeq) throws SQLException {
		Map<String, Object> pojo_bean = obj.listInsertableFields();
		String[] fields = pojo_bean.keySet().toArray(
				new String[pojo_bean.size()]);
		String sql = getOracleInsertSql(obj,indexSeq);
		PreparedStatement ps = null;
		ResultSet rs = null;
		Connection conn=getConnection();
		int r=-1;
	...
	private static String getOracleInsertSql(Pojo obj,String indexSeq) {
		try {
			String insertSql = insertSqlCache.get(obj.getClass().getName());
			if (insertSql == null) {
				Map<String, Object> pojo_bean = obj.listInsertableFields();
